...
- **Replace references in selection.** Pick GameObjects, the window lists every Object reference inside their VRCFury components — for each row, drag a replacement into the *Replace* slot and click Apply. *All like this* sets the same replacement on every row whose current value matches, so renaming a bone everywhere is one drag + one click. One Undo step. *(Tools → VRCFury QoL → Replace References…, or right-click a selection → VRCFury QoL → Replace references in selection…)*
